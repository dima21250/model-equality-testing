...
Processes in batches to avoid timeouts and writes incrementally.
"""
...
def process_batch(
    model: str,
    dataset: str,
    prompt_ids_range: Tuple[int, int],
    sources: List[str],
    L: int,
    samples: int,
    root_dir: str,
    ...
    """
    Process a batch of prompt IDs and return the markdown rows for the batch
    and a flag indicating if any low p-value was found.
    """
    start, end = prompt_ids_range
    rows = []
    any_low_p = False
    for prompt_id in range(start, end + 1):
        ...
        for source in sources:
            ...
            samp_a = dist.sample(n=samples)
            samp_b = dist.sample(n=samples)
            ks_stat, ks_p, elapsed = compute_ks(samp_a, samp_b)
            note = ""
            if ks_p < 0.05:
                note = "⚠️ Low p-value (possible false rejection)"
                any_low_p = True
            rows.append(f"| {prompt_id} | {source} | {ks_stat:.6f} | {ks_p:.6f} | {note} |")
    return "\n".join(rows), any_low_p
...
def main():
    ...
    with open(args.output, "a") as f:
        for batch_start in range(args.start_prompt, args.end_prompt + 1, args.batch_size):
            batch_end = min(batch_start + args.batch_size - 1, args.end_prompt)
            print(f"Processing prompts {batch_start} to {batch_end}...")
            batch_rows, batch_low_p = process_batch(
                model=args.model,
                dataset=args.dataset,
                prompt_ids_range=(batch_start, batch_end),
