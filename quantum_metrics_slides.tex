...
\textbf{Convert LLM outputs to quantum states:}

\begin{enumerate}
  \item Embed completions using sentence transformers (MPNet, EmbeddingGemma-300M)
  \item Construct density matrices: $\rho = \frac{1}{N}\sum_i |\psi_i\rangle\langle\psi_i|$
  \item Compute quantum information metrics:
  \begin{itemize}
    \item \textbf{Trace distance}: $D(\rho_A, \rho_B) = \frac{1}{2}\text{Tr}|\rho_A - \rho_B|$
    \item \textbf{Von Neumann entropy}: $S(\rho) = -\text{Tr}(\rho \log \rho)$
    \item \textbf{Quantum relative entropy}: $D(\rho_A || \rho_B) = \text{Tr}(\rho_A \log \rho_A - \rho_A \log \rho_B)$
  \end{itemize}
\end{enumerate}

\vspace{0.5em}
\textbf{Hypothesis:} Quantum metrics provide interpretable characterization beyond p-values
