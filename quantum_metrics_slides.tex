...
\textbf{Goal:} Quantum-inspired metrics for interpretable LLM distribution characterization

\vspace{0.3em}

\textbf{What we tried:} Embeddings $\to$ Gram matrix $\to$ density matrix $\to$ quantum metrics (trace distance, Von Neumann entropy, QRE)

\vspace{0.3em}

\textbf{What happened:}
\begin{enumerate}
  \item Initial promising: trace distance = 0.32 for fp32 vs int8
  \item \textcolor{darkred}{\textbf{Sanity check failure:}} fp32 vs fp32 = 0.33 (same!)
  \item \textcolor{darkred}{\textbf{Root cause:}} Sample-size artifacts
  \item \textcolor{darkgreen}{\textbf{PCA fix:}} Artifacts eliminated, but 7\% discrimination
  \item \textcolor{darkgreen}{\textbf{Pivot:}} Semantic embeddings reveal what doesn't change
\end{enumerate}
...
\begin{itemize}
  \item Semantic invariance: quantization preserves semantics
  \item Two-tier framework: token-level detection + semantic impact
  \item Language-dependent convergence: corpus standardization matters
\end{itemize}
...
\textbf{Quantum metrics for detection:} \textcolor{darkred}{\textbf{Not viable}} — sample-size artifacts, weak discrimination, no signal in embedding space

\vspace{0.3em}

\textbf{Semantic embeddings for interpretability:} \textcolor{darkgreen}{\textbf{Valuable insight}}
\begin{itemize}
  \item Implementation changes preserve semantics (quantization, architecture)
  \item Two-tier framework: "Did it change?" + "Does it matter?"
  \item Separation ratios reveal training corpus standardization
\end{itemize}

\vspace{0.3em}

\textbf{Unexpected finding:} Language-dependent convergence
\begin{itemize}
  \item French 0.19 (canonical sources) vs Spanish 1.00 (dialectal variation)
  \item NOT explained by data abundance or organizational bias
\end{itemize}

\vspace{0.3em}

\textbf{Practical value:} Deployment decision framework
\begin{itemize}
  \item Statistical change + semantic preservation = likely safe
  \item Example: fp32$\to$int8 detected (p=0.01) + preserved (0.0) = safe
\end{itemize}
