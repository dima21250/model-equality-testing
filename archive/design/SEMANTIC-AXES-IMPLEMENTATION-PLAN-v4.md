# Plan: Semantic Axes Interpretation Layer (v4)
...
**`model_equality_testing/src/semantic_axes.py`** — single new module, no changes to existing code. Uses `logging.warning()` for all warnings to match the convention in `embeddings.py`.
...
Custom `__repr__` returns a compact summary (no 768-dim array dumps):
```
...
metadata = {
    ...
    "negative_pole_intra_std": float,     # RMS distance of embeddings from centroid (negative pole)
    "positive_pole_intra_std": float,     # RMS distance of embeddings from centroid (positive pole)
    "separation_ratio": float,            # pole_distance / mean(intra_std_neg, intra_std_pos)
    "created_at": str,                    # ISO 8601 timestamp
}
```

**Intra-pole standard deviation** is the root-mean-square of L2 distances from the centroid:
```
intra_std = sqrt(mean(||x_i - centroid||²))
```
This is genuine standard deviation (not mean L2 distance), matching the `_intra_std` field name.
...
d = (mean_a - mean_b) / s_avg           # guard: if s_avg < 1e-10, d = 0.0
J = 1 - 3 / (4 * (n_a + n_b) - 9)      # guard: if n_a + n_b < 4, J = 1.0
hedges_g = d * J
```
...
- Extract pole names from `metadata.concept` in the first record of each file; if the field is missing, derive the pole name from the filename (e.g., `professionalism.jsonl` → `"professionalism"`)
...
- Compute intra-pole standard deviation: `sqrt(mean(||x_i - centroid||²))` (RMS of deviations)
- Compute `pole_distance = ||centroid_pos - centroid_neg||`
- Compute `separation_ratio = pole_distance / max(mean(intra_std_neg, intra_std_pos), 1e-10)`
...
- `logging.warning()` if either corpus has fewer than 50 samples
- `logging.warning()` if `separation_ratio < 1.0` (poles overlap substantially)
...
**`save_axes(axes: List[SemanticAxis], directory: str)`** — Create directory if it doesn't exist. Validate that axis names are unique (error on duplicates). Save each axis as `{directory}/{sanitized_name}.npz`. Name sanitization: replace characters not in `[a-zA-Z0-9_-]` with `_`.

**`load_axes(directory: str) → List[SemanticAxis]`** — Load all `.npz` files from the directory, sorted alphabetically by filename for deterministic ordering.
...
- For each axis compute:
  ...
  - Delta = mean_a - mean_b
  - Hedges' g using average-SD denominator (see formula in AxisProjectionResult); guarded for s_avg ≈ 0 and small n
  - Welch's t-test via `scipy.stats.ttest_ind(equal_var=False)`
  - 1D Wasserstein distance via `scipy.stats.wasserstein_distance`
- Apply Benjamini-Hochberg FDR correction across all axes via `_benjamini_hochberg()`
- For K=1 axis, corrected p-value equals raw p-value (no correction needed)
- Return SemanticInterpretation with all results

**`_benjamini_hochberg(pvalues) → np.ndarray`** — internal helper:
```python
...
- Validates embedding model name matches axis model (`logging.warning()` on mismatch) — this is the correct scope for model validation since this function knows the model name
- Follows the `_precomputed_embeddings` pattern from `tests.py`

### Axis Correlations

**`axis_correlations(axes: List[SemanticAxis]) → np.ndarray`** — cosine similarity matrix between axis vectors. `logging.warning()` if any pair correlates above 0.7 (indicates overlapping dimensions — interpretations may overcount differences).
...
1. `SemanticAxis` dataclass with validation (dimension, unit norm, pole distance) and compact `__repr__`
2. `_load_jsonl_texts()` helper to read JSONL and extract text; fall back to filename-derived pole name if `metadata.concept` is missing
3. `load_axis_from_jsonl()` — load corpora, embed, L2-normalize, compute centroids and axis vector, validate separation, compute intra-pole RMS std and separation ratio (with epsilon guard on denominator)
...
5. `AxisProjectionResult` dataclass with SEM, Hedges' g (average-SD denominator + guarded J correction), corrected p-value
6. `_benjamini_hochberg()` helper with `np.asarray()` cast, K=1 passthrough, monotonicity enforcement
...
10. `save_axes()` / `load_axes()` — directory of per-axis npz files, sanitized filenames, alphabetical load order, duplicate name check, auto-create directory
11. `interpret_samples()` — CompletionSample convenience wrapper with model name validation via `logging.warning()`
12. `axis_correlations()` — cosine similarity matrix with warnings via `logging.warning()`
...
3. **Edge case tests**: Near-zero pole distance (should error), dimension mismatch (should error), empty corpus (should error), single axis BH correction (corrected = raw), missing `metadata.concept` (should fall back to filename), s_avg ≈ 0 (hedges_g = 0), n_a + n_b < 4 (J = 1.0)
...
6. **Serialization tests**: Filename sanitization (e.g., `"a/b:c"` → `"a_b_c"`), duplicate name detection (should error), directory auto-creation, alphabetical load ordering
7. **Integration test** (optional, skip if semantic-pole not present): Load professionalism/casualness corpora from `~/Code/semantic-pole/output/`, build axis, verify separation ratio > 1.0
8. **End-to-end**: Run quantum detection (trace distance) + semantic interpretation on fp32 vs int8 Llama-3-8B, verify interpretable output with correct labels and FDR markers
